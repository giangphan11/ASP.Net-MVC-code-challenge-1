  ...
  public  class Orders_Info
    {
      ...
      // thêm 1 đơn đặt hàng mới
        ...
        public static int Add(int id,string TenKhachHang, string DiaChi, string Email, string Phone, string GhiChu,double TongTien, string status)
        {
        ...
        public static List<Orders_Info> GetXuLy(string trangthai)
        {
            return CBO.FillCollection<Orders_Info>(DataProvider.Instance.ExecuteReader("Orders_GetTrangThai",trangthai));
        }

        public static List<OrderDetails_Info> GetListOrdetails(int id)
        {
            return CBO.FillCollection<OrderDetails_Info>(DataProvider.Instance.ExecuteReader("get_list_product_by_orderid", id));
        }
    }
}
